...
import com.dxyt.recorded.listener.CaptureListener;
import com.dxyt.recorded.listener.ClickListener;
import com.dxyt.recorded.listener.ErrorListener;
import com.dxyt.recorded.listener.YTCameraListener;
import com.dxyt.recorded.listener.TypeListener;
import com.dxyt.recorded.state.CameraMachine;
import com.dxyt.recorded.util.FileUtil;
import com.dxyt.recorded.util.LogUtil;
import com.dxyt.recorded.util.ScreenUtils;
import com.dxyt.recorded.view.CameraView;
 ...
 * =====================================
 */
public class YTCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, CameraView {
//    private static final String TAG = "JCameraView";
    ...
    //回调监听
    private YTCameraListener jCameraLisenter;
    ...
    public YTCameraView(Context context) {
        this(context, null);
    }

    public YTCameraView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public YTCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        //get AttributeSet
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.YTCameraView, defStyleAttr, 0);
        iconSize = a.getDimensionPixelSize(R.styleable.YTCameraView_iconSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 35, getResources().getDisplayMetrics()));
        iconMargin = a.getDimensionPixelSize(R.styleable.YTCameraView_iconMargin, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
        iconSrc = a.getResourceId(R.styleable.YTCameraView_iconSrc, R.mipmap.video_camera);
        iconLeft = a.getResourceId(R.styleable.YTCameraView_iconLeft, 0);
        iconRight = a.getResourceId(R.styleable.YTCameraView_iconRight, 0);
        duration = a.getInteger(R.styleable.YTCameraView_duration_max, 10 * 1000);       //没设置默认为10s
        a.recycle();
        initData();
        initView();
    }
...
//        });
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        new Thread() {
            @Override
            public void run() {
                CameraInterface.getInstance().doOpenCamera(YTCameraView.this);
            }
        }.start();
    }
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public void setJCameraLisenter(YTCameraListener jCameraLisenter) {
    ...
    public void playVideo(Bitmap firstFrame, final String url) {
        videoUrl = url;
        YTCameraView.this.firstFrame = firstFrame;
